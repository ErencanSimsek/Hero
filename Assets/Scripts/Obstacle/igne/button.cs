...
public class button : MonoBehaviour
{
    ...
    private void Update()
    {
            ...
            Vector3.Distance(transform.localPosition, Player.transform.localPosition) <= 1.3f)
        {
            ...
            for(int i = 1; i <= 16; i++)
            {
                GameObject.FindGameObjectWithTag("igne"+i).GetComponent<SpriteRenderer>().enabled = false;
                GameObject.FindGameObjectWithTag("igne"+i).GetComponent<PolygonCollider2D>().enabled = false;
            }
            active = 1;
        }
